<script lang="ts">
    ...
    import {
        workspaceShellContextKey,
        type WorkspaceShellContext,
    } from "$lib/components/app/workspace-shell-context.js";
    import type {
        CommandAction,
        Comment,
        SyncState,
    ...
    const workspaceShellContext: WorkspaceShellContext = {
        get boards() {
            return boards.boards;
        },
        get activeBoardId() {
            return activeBoard?.id ?? null;
        },
        get syncState() {
            return syncState;
        },
        ...
        onCreateTicketForBoard: (boardId) => {
            void createTicketForBoard(boardId);
        },
    ...
    setContext(workspaceShellContextKey, workspaceShellContext);
</script>
...
{:else if workspace.status === "no_workspace"}
    <div class="flex h-full items-center justify-center p-8">
        <div
            class="w-full max-w-md space-y-3 rounded-xl border border-border/80 bg-surface-panel/80 p-5 text-center"
        >
            <h1 class="font-display text-lg font-semibold">Redirecting...</h1>
            <p class="text-sm text-muted-foreground">
                No workspace selected. Sending you to onboarding.
            </p>
            <Button onclick={() => void goto("/onboarding")}
